...
# Function to group songs and user by count and check it agains mean of target #
...
##### Analysis of outputs #####
output<-read.csv("predictions.csv")

## check the distribution of target ##
output %>% 
  group_by(target) %>% 
  count
## Test column count and its effect on target ##

# source_system_tab #
target_vs_column(output, col_name = "source_system_tab",
                 x = 'Frequency',
                 y = 'Target',
                 title = 'Count of source_system_tab vs Target')

# source_screen_name #
target_vs_column(output, col_name = "source_screen_name",
                 x = 'Frequency',
                 y = 'Target',
                 title = 'Count of source_screen_name vs Target')

# source_type #
target_vs_column(output, col_name = "source_type",
                 x = 'Frequency',
                 y = 'Target',
                 title = 'Count of source_type vs Target')
